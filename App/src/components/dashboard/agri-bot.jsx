...
const API_URL = "http://127.0.0.1:5005/chat"; // Make sure this matches your server port!

export function AgriBot() {
  const { t, lang } = useLanguage(); // 2. GET LANG
  
  // 3. USE t() FOR INITIAL MESSAGE
  const [messages, setMessages] = useState([]);

  // Reset chat when language changes to show correct greeting
  useEffect(() => {
    setMessages([{ role: "bot", text: t("bot_intro") }]);
  }, [lang]); 
    ...
    <Card className="h-[600px] flex flex-col border-emerald-100 shadow-lg bg-white">
      <CardHeader className="bg-emerald-600 text-white py-4 rounded-t-xl">
        <CardTitle className="flex items-center gap-3 text-lg font-medium">
            ...
            <div>
                <p>{t("bot_name")}</p>
                <p className="text-xs text-emerald-100 font-normal">{t("bot_powered")}</p>
            </div>
        </CardTitle>
      </CardHeader>
      ...
      <CardFooter className="p-3 bg-white border-t">
        <div className="flex w-full gap-2 relative">
            ...
            />
            <Button onClick={handleSend} size="icon" disabled={loading} className="absolute right-1 top-1 h-8 w-8 rounded-full bg-emerald-600 hover:bg-emerald-700">
